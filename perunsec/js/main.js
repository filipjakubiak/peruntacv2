...
(function () {
  ...
  if (langSwitch) {
    langSwitch.addEventListener("click", function (e) {
      /* Ręczny wybór języka wygrywa z auto-detekcją (patrz snippet w <head>) */
      try { localStorage.setItem("perun-lang", EN ? "pl" : "en"); } catch (err) {}
  ...
  var artToc = document.querySelector(".art-toc");
  if (artToc && "IntersectionObserver" in window) {
    var tocLinks = artToc.querySelectorAll("a[href^=\"#\"]");
    var tocMap = {};
    tocLinks.forEach(function (a) { tocMap[a.getAttribute("href").slice(1)] = a; });
    var sections = Array.prototype.slice.call(document.querySelectorAll(".art h2[id]"));
    var current = null;
    function setActive(id) {
      if (id === current) return;
      if (current && tocMap[current]) tocMap[current].classList.remove("is-active");
      current = id;
      if (current && tocMap[current]) tocMap[current].classList.add("is-active");
    }
    /* A heading "activates" its section as it crosses a trigger line near the
       top of the viewport, and STAYS active while the reader scrolls through
       the body text under it — only the next heading crossing that line
       hands off the highlight. (Only reacting to isIntersecting:true, and
       never clearing on exit, is what gives that hold-until-next-heading
       behavior — a plain "is it visible right now" check would leave nothing
       highlighted for most of each section's body text.) */
    ...
    sections.forEach(function (h) { io.observe(h); });
  }
